...
<<<<<<< HEAD
	...
	while (1)
    {
        ...
        if (emptyLine(input_line) == 1) /* 1 is true */
            continue; /* Go back to the beginning of the loop (printPrompt again) */        /* Parse the input line and store them in command_arguments array*/
        command_arguments = parseLine(input_line);        /* Check if the user wants to exit the shell */
